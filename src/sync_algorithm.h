...
ColumnValues extend_last_key(DatabaseClient &client, const Table &table, const ColumnValues &last_key) {
	...
	return row_last_key.last_key; // may be empty if we have no more rows
}

template <typename Worker, typename DatabaseClient>
void check_hash_and_choose_next_range(Worker &worker, DatabaseClient &client, const Table &table, const ColumnValues &prev_key, const ColumnValues &last_key, const string &hash) {
	...
	if (hasher_for_their_range.finish() == hash) {
		...
		find_hash_of_next_range(worker, client, table, hasher_for_their_range.row_count*2, last_key);

	} else if (hasher_for_their_range.row_count > 1) {
		// no match, try again starting at the same row for a smaller set of rows
		find_hash_of_next_range(worker, client, table, hasher_for_their_range.row_count/2, prev_key);
		...
		if (hasher_for_their_range.row_count == 0 && !last_key.empty()) {
			extended_last_key = extend_last_key(client, table, last_key); // note that it may still be []
		} else {
