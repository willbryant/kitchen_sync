...
inline void choose_primary_key_for(Table &table) {
	...
	ColumnNullableOrReplaced unusable(table);
	...
	for (const Key &key : table.keys) {
		if (key.unique() && none_of(key.columns.begin(), key.columns.end(), unusable)) {
	...
	if (none_of(table.columns.begin(), table.columns.end(), unusable)) {
