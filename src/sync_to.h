...
struct SyncToWorker {
	...
	void handle_hash_next_command(const Table &table) {
		...
		if (verbose >= VERY_VERBOSE) cout << "-> hash " << table.name << ' ' << non_binary_string_values_list(client, prev_key) << ' ' << non_binary_string_values_list(client, last_key) << endl;
	...
	void handle_hash_fail_command(const Table &table) {
		...
		if (verbose >= VERY_VERBOSE) cout << "-> hash " << table.name << ' ' << non_binary_string_values_list(client, prev_key) << ' ' << non_binary_string_values_list(client, last_key) << " last-failure " << non_binary_string_values_list(client, failed_last_key) << endl;
	...
	bool handle_rows_command(const Table &table, TableRowApplier<DatabaseClient> &row_applier) {
		...
		if (verbose >= VERY_VERBOSE) cout << "-> rows " << table.name << ' ' << non_binary_string_values_list(client, prev_key) << ' ' << non_binary_string_values_list(client, last_key) << endl;
	...
	void handle_rows_and_hash_next_command(const Table &table, TableRowApplier<DatabaseClient> &row_applier) {
		...
		if (verbose >= VERY_VERBOSE) cout << "-> rows " << table.name << ' ' << non_binary_string_values_list(client, prev_key) << ' ' << non_binary_string_values_list(client, last_key) << " +" << endl;
		if (verbose >= VERY_VERBOSE) cout << "-> hash " << table.name << ' ' << non_binary_string_values_list(client, last_key) << ' ' << non_binary_string_values_list(client, next_key) << endl;
	...
	void handle_rows_and_hash_fail_command(const Table &table, TableRowApplier<DatabaseClient> &row_applier) {
		...
		if (verbose >= VERY_VERBOSE) cout << "-> rows " << table.name << ' ' << non_binary_string_values_list(client, prev_key) << ' ' << non_binary_string_values_list(client, last_key) << " +" << endl;
		if (verbose >= VERY_VERBOSE) cout << "-> hash " << table.name << ' ' << non_binary_string_values_list(client, last_key) << ' ' << non_binary_string_values_list(client, next_key) << " last-failure " << non_binary_string_values_list(client, failed_last_key) << endl;
